...
export default async function handler(req, res) {
  ...
  const maxResults = max ? parseInt(max, 10) || 10 : 10;
  ...
  try {
    ...
    if (!r.ok) {
      console.error("Gmail list-latest error", data);
      return res.status(200).json({
        status: "error",
        message: "Error listando correos en Gmail API",
        data: null
      });
    }
    ...
    return res.status(200).json({
      status: "success",
      message: "Listado de correos recientes obtenido correctamente",
      data: { messages: enriched }
    });
  } catch (error) {
    console.error("list-latest handler error", error);
    return res.status(200).json({
      status: "error",
      message: "Error interno listando correos",
      data: null
    });
  }
}
