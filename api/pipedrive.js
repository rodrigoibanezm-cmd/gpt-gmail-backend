...
module.exports = async (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case "createDeal": {
        if (!dealData?.title) {
          return res.status(400).json({ status: "error", message: "title requerido" });
        }
      ...
      case "moveDealStage": {
        ...
        const r = await pipedriveRequest("PUT", `/deals/${dealId}`, { body: { stage_id: stageId } });
      ...
      case "addNote": {
        ...
        const r = await pipedriveRequest("POST", "/notes", { body: { deal_id: dealId, content: noteText } });
        return res.status(200).json(r);
      }
