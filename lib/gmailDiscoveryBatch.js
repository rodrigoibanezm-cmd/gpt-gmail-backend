...
const SUMMARY_LEVELS = new Set(["minimal", "compact", "full"]);

async function fetchCompactThread(accessToken, threadId, ownerEmail, maxLatest, summaryLevel) {
  ...
  return compactThread(data, ownerEmail, maxLatest, summaryLevel);
}

function normalizeSummaryLevel(value) {
  const level = String(value || "compact").toLowerCase();
  return SUMMARY_LEVELS.has(level) ? level : "compact";
}
...
function normalizeParams(params = {}) {
  return {
    ...
    summaryLevel: normalizeSummaryLevel(params.summaryLevel)
  };
}
...
export async function discoveryBatch(userId, params = {}) {
  ...
  try {
    ...
    for (; index < found.threadIds.length; index += 1) {
      const t = await fetchCompactThread(
        accessToken,
        found.threadIds[index],
        ownerEmail,
        cfg.maxLatestMessages,
        cfg.summaryLevel
      );
