...
function participants(messages) {
  ...
  for (const msg of messages) {
    all.push(msg.from, ...splitPeople(msg.to), ...splitPeople(msg.cc));
  }
  return unique(all).slice(0, 30);
}

export function compactThread(data = {}, ownerEmail, maxLatest = 4) {
  ...
  const carolinaMessages = messages.filter((m) => m.from_owner).map((m) => ({
    id: m.id,
    date: m.date,
    snippet: m.snippet
  }));
