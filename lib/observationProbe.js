...
async function fetchThread(accessToken, threadId, ownerEmail) {
  ...
  if (!r.ok) return { conversation_id: threadId, error: "thread_fetch_failed", identities: [] };
  ...
  for (let i = 0; i < sourceMessages.length; i += 1) {
    const normalized = normalizeMessage(sourceMessages[i], ownerEmail, i + 1, events[i - 1]);
    events.push(normalized.event);
    identities.push(...normalized.identities);
  }

  const participantIds = uniqueIds(events.flatMap((event) => [event.sender_id, ...event.to_ids, ...event.cc_ids]));
...
export async function observationProbe(userId, params = {}) {
  ...
  for (const threadId of found.threadIds) {
    ...
    const { identities, ...conversation } = result;
  ...
  return {
    ok: true,
    probe_version: "observation_probe.v0.1",
