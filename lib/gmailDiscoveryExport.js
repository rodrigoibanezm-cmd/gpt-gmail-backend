...
function slimMessage(m, maxChars) {
  const txt = String(m.body_text || "");
  return {
    ...m,
    body_text: txt.slice(0, maxChars),
    body_truncated: m.body_truncated || txt.length > maxChars
  };
}

function fitThread(thread, budget) {
  if (JSON.stringify(thread).length <= budget) return thread;
  ...
  while (chars >= 300) {
    const candidate = {
      ...
      messages: (thread.messages || []).map((m) => slimMessage(m, chars))
    };
  ...
  return {
    ...
    messages_returned: thread.messages_returned || 0,
    payload_truncated: true,
    messages: (thread.messages || []).map((m) => slimMessage(m, 300))
  };
}
...
export async function exportGmailDiscovery(userId, params = {}) {
  ...
  const maxThreads = n(params.maxThreads, 10, 25);
  const maxBodyChars = n(params.maxBodyChars, 6000, 15000);
  const maxPayloadChars = n(params.maxPayloadChars, 90000, 180000);
  ...
  try {
    ...
    for (; index < found.threadIds.length; index += 1) {
      const budget = Math.max(2000, maxPayloadChars - used);
      const raw = await getThread(accessToken, found.threadIds[index], maxBodyChars);
      const thread = fitThread(raw, budget);
      const size = JSON.stringify(thread).length;
      if (threads.length && used + size > maxPayloadChars) break;
      if (!threads.length && size > maxPayloadChars) break;
      threads.push(thread);
      used += size;
    }
